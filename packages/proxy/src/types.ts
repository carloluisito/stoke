...
export type PauseReason =
  ...
  /**
   * A Claude Code hook reported the owning session ended. Unlike every other
   * reason this is a certainty rather than an inference: return probability is
   * exactly 0, so no further ping can ever pay off.
   */
...
export interface Session {
  ...
  /**
   * Claude Code's own session_id, recovered from the `<stoke-session>` marker the
   * UserPromptSubmit hook injects. This is the only join between a hook's view
   * of the world (session_id) and ours (a hash of model + tools + system).
   *
   * Absent for subagents (fresh message array, so no marker) and for installs
   * without the plugin — both fall through to the hook-less behavior.
   */
  claudeSessionId?: string;
  /**
   * Whether the `pings_without_progress` pause happened while the session was
   * idle at the prompt (speculative) rather than mid-turn (return was certain).
   * Only speculative pauses are evidence about whether idle users come back, so
   * only they feed the adaptive cap. `undefined` counts as speculative, which
   * preserves pre-hook behavior.
   */
  pausedWhileIdle?: boolean;
}

/**
 * Lifecycle signals published by the Claude Code plugin hooks. Optional: with no
...
export interface Config {
  ...
  /**
   * Claude Code session lifecycle signals. Optional so existing configs keep
   * validating; when absent the scheduler simply has no signal and behaves as it
