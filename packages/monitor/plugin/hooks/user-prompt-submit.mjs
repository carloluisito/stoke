...
try {
  ...
  if (turns.length > 0) {
    ...
    if (on("cache_expiry_warning") && hadCache && gapMs > ttlMs) {
      const ctxK = Math.round((last.cache_read + last.input_tokens) / 1000);
    ...
    if (on("context_bloat_warning") && meanContext > (cfg.thresholds.bloatContextTokens || 120000)) {
      const ctxK = Math.round(meanContext / 1000);
